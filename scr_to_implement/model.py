...
class ResBlock(nn.Module):
    ...
    def __init__(self,in_channel,out_channel,stride):
        ...
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv2d(out_channel,out_channel,kernel_size=3,stride=None)
        self.bn2 = nn.BatchNorm2d(out_channel)
        self.conv_matching = nn.Conv2d(in_channel,out_channel,kernel_size=1)
    ...
    def forward(self,x):
        ...
        x = self.conv2(x)
        x = self.bn2(x)
        skipped_connection = ResBlock.layer_matching(self.input)
...
class ResNet(nn.Module):
    ...
    def __init__(self):
        ...
        #self.globalaverage = nn.AvgPool2d()
        #self.flatten = ????????????????????????????????????????????????????????????????????????
